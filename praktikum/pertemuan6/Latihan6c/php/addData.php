<?php

require 'functions.php';

session_start();

if(!isset($_SESSION["username"])) {
    header("Location: login.php");
    exit;
}

if (isset($_POST['addData'])) {
    if (addData($_POST) > 0) {
        echo "
            <script>
                alert('Success');
                document.location.href = 'admin.php';
            </script>";
    } else {
        echo "
            <script>
                alert('Failed');
                document.location.href = 'admin.php';
            </script>";
    }
}

?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Add Data</title>
    <link rel="icon" href="data:image/png;base64,"/>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-BmbxuPwQa2lc/FVzBcNJ7UAyJxM6wuqIj61tLrc4wSX0szH/Ev+nYRRuWlolflfl" crossorigin="anonymous">
</head>
<body>
    <div class="container pt-5">
        <form action="" method="post">
            <div class="form-row">
                <div class="form-group col-md-4 mb-2">
                    <label for="itemsName">Item Name</label>
                    <input type="text" class="form-control" name="itemsName" id="itemsName" placeholder="">
                </div>
                <div class="form-group col-md-4 mb-2">
                    <label for="itemsColor">Color</label>
                    <input type="text" class="form-control" name="itemsColor" id="itemsColor" placeholder="">
                </div>
                <div class="form-group col-md-4 mb-2">
                    <label for="itemsPrice">Price</label>
                    <input type="text" class="form-control" name="itemsPrice" id="itemsPrice" placeholder="">
                </div>
                <div class="form-group col-md-4 mb-2">
                    <label for="itemsStock">Stock</label>
                    <input type="text" class="form-control" name="itemsStock" id="itemsStock" placeholder="">
                </div>
                <div class="form-group col-md-4 mb-4">
                    <label for="itemsIMG">Image File</label>
                    <input type="text" class="form-control" name="itemsIMG" id="itemsIMG" placeholder="">
                </div>
            </div>
            <button type="submit" class="btn btn-success " name="addData">Submit</button>
            <a href="admin.php" class="btn btn-danger">Back</a>
        </form>
    </div>


    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta2/dist/js/bootstrap.bundle.min.js" integrity="sha384-b5kHyXgcpbZJO/tY9Ul7kGkf1S0CWuKcCD38l8YkeH8z8QjE0GmW1gYU5S9FOnJ0" crossorigin="anonymous"></script>
    <script src="https://code.jquery.com/jquery-3.5.1.js"></script>
    <script src="https://unpkg.com/boxicons@latest/dist/boxicons.js"></script>
</body>
</html>